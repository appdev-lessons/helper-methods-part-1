...
- The project was previously called `helper-methods`, but it has been renamed to `helper-methods-1-and-2`
...
The staring point code for `helper-methods-1-and-2` is where you should have arrived by the end of the previous `getting-started-with-scaffolds` project. The current project `helper-methods-1-and-2` covers everything in this lesson and the next lesson _Helper Methods Part 2_, so you will need to keep it open when you move onto that lesson in order to solve all of the specs and get full credit.
